...
import io.github.jhipster.sample.service.EmployeeService;
import io.github.jhipster.sample.service.dto.EmployeeCriteria;
import io.github.jhipster.sample.service.EmployeeQueryService;
...
public class EmployeeResourceIT {
    ...
    private static final ZonedDateTime SMALLER_HIRE_DATE = ZonedDateTime.ofInstant(Instant.ofEpochMilli(-1L), ZoneOffset.UTC);
    ...
    public static Employee createEntity(EntityManager em) {
        ...
        if (TestUtil.findAll(em, SilverBadge.class).isEmpty()) {
            silverBadge = SilverBadgeResourceIT.createEntity(em);
            em.persist(silverBadge);
            em.flush();
        } else {
            silverBadge = TestUtil.findAll(em, SilverBadge.class).get(0);
        }
        employee.setSibag(silverBadge);
        // Add required entity
        GoldenBadge goldenBadge;
        if (TestUtil.findAll(em, GoldenBadge.class).isEmpty()) {
            goldenBadge = GoldenBadgeResourceIT.createEntity(em);
            em.persist(goldenBadge);
            em.flush();
        } else {
            goldenBadge = TestUtil.findAll(em, GoldenBadge.class).get(0);
        }
    ...
    public static Employee createUpdatedEntity(EntityManager em) {
        ...
        SilverBadge silverBadge;
        if (TestUtil.findAll(em, SilverBadge.class).isEmpty()) {
            silverBadge = SilverBadgeResourceIT.createUpdatedEntity(em);
            em.persist(silverBadge);
            em.flush();
        } else {
            silverBadge = TestUtil.findAll(em, SilverBadge.class).get(0);
        ...
        GoldenBadge goldenBadge;
        if (TestUtil.findAll(em, GoldenBadge.class).isEmpty()) {
            goldenBadge = GoldenBadgeResourceIT.createUpdatedEntity(em);
            em.persist(goldenBadge);
            em.flush();
        } else {
            goldenBadge = TestUtil.findAll(em, GoldenBadge.class).get(0);
    ...
    public void createEmployee() throws Exception {
        ...
        assertThat(testEmployee.getId()).isEqualTo(testEmployee.getUser().getId());
    }
    ...
    @Test
    @Transactional
    public void updateEmployeeMapsIdAssociationWithNewId() throws Exception {
    ...
    @Test
    @Transactional
    public void getAllEmployeesBySibagIsEqualToSomething() throws Exception {
        // Get already existing entity
        SilverBadge sibag = employee.getSibag();
        employeeRepository.saveAndFlush(employee);
        Long sibagId = sibag.getId();

        // Get all the employeeList where sibag equals to sibagId
        defaultEmployeeShouldBeFound("sibagId.equals=" + sibagId);

        // Get all the employeeList where sibag equals to sibagId + 1
        defaultEmployeeShouldNotBeFound("sibagId.equals=" + (sibagId + 1));
    }


    @Test
    @Transactional
    public void getAllEmployeesByGobagIsEqualToSomething() throws Exception {
        // Get already existing entity
        GoldenBadge gobag = employee.getGobag();
        employeeRepository.saveAndFlush(employee);
        Long gobagId = gobag.getId();

        // Get all the employeeList where gobag equals to gobagId
        defaultEmployeeShouldBeFound("gobagId.equals=" + gobagId);

        // Get all the employeeList where gobag equals to gobagId + 1
        defaultEmployeeShouldNotBeFound("gobagId.equals=" + (gobagId + 1));
    }
    ...
    @Test
    @Transactional
    public void getAllEmployeesByHistoryIsEqualToSomething() throws Exception {
        // Initialize the database
        employeeRepository.saveAndFlush(employee);
        JobHistory history = JobHistoryResourceIT.createEntity(em);
        em.persist(history);
        em.flush();
        ...
        // Get all the employeeList where history equals to historyId
        defaultEmployeeShouldBeFound("historyId.equals=" + historyId);

        // Get all the employeeList where history equals to historyId + 1
        defaultEmployeeShouldNotBeFound("historyId.equals=" + (historyId + 1));
    }
