...
export class OperationUpdatePage {
  ...
  theLabelSelect = element(by.id('field_theLabel'));
  ...
  async theLabelSelectLastOption(): Promise<void> {
    await this.theLabelSelect.all(by.tagName('option')).last().click();
  }

  async theLabelSelectOption(option: string): Promise<void> {
    await this.theLabelSelect.sendKeys(option);
  }

  getTheLabelSelect(): ElementFinder {
    return this.theLabelSelect;
  }

  async getTheLabelSelectedOption(): Promise<string> {
    return await this.theLabelSelect.element(by.css('option:checked')).getText();
  }
