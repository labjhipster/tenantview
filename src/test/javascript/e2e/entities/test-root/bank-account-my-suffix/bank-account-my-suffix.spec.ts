...
import {
  BankAccountComponentsPage,
  /* BankAccountDeleteDialog, */
  BankAccountUpdatePage,
} from './bank-account-my-suffix.page-object';
...
describe('BankAccount e2e test', () => {
  ...
  /* it('should create and save BankAccounts', async () => {
        ...
        await promise.all([
            ...
            bankAccountUpdatePage.companySelectLastOption(),
        ...
        expect(await bankAccountUpdatePage.getLastOperationDurationInput()).to.eq('5', 'Expected lastOperationDuration value to be equals to 5');
        expect(await bankAccountUpdatePage.getMeanOperationDurationInput()).to.eq('5', 'Expected meanOperationDuration value to be equals to 5');
        ...
        expect(await bankAccountUpdatePage.getLastOperationDateInput()).to.contain('2001-01-01T02:30', 'Expected lastOperationDate value to be equals to 2000-12-31');
        ...
        expect(await bankAccountUpdatePage.getAttachmentInput()).to.endsWith(fileNameToUpload, 'Expected Attachment value to be end with ' + fileNameToUpload);
        expect(await bankAccountUpdatePage.getDescriptionInput()).to.eq('description', 'Expected Description value to be equals to description');
