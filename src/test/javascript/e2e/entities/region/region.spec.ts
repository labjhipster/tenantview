...
describe('Region e2e test', () => {
  ...
  it('should create and save Regions', async () => {
    ...
    await promise.all([regionUpdatePage.setNameInput('name'), regionUpdatePage.countrySelectLastOption()]);

    expect(await regionUpdatePage.getNameInput()).to.eq('name', 'Expected Name value to be equals to name');
