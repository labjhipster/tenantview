...
describe('Job e2e test', () => {
  ...
  it('should create and save Jobs', async () => {
    ...
    await promise.all([
      jobUpdatePage.setTitleInput('title'),
      jobUpdatePage.typeSelectLastOption(),
      jobUpdatePage.setMinSalaryInput('5'),
      jobUpdatePage.setMaxSalaryInput('5'),
      // jobUpdatePage.choreSelectLastOption(),
      jobUpdatePage.empSelectLastOption(),
    ]);

    expect(await jobUpdatePage.getTitleInput()).to.eq('title', 'Expected Title value to be equals to title');
